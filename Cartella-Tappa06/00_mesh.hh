...
private:
    ...
    // Le dimensioni ora sono variabili!
    int m_width;
    int m_depth;
...
public:
    Mesh(int width, int depth, int octaves, float frequency, float amplitude, unsigned int seed, bool use_dem, std::string dem_file = "aletsch.asc") {
        ...
        if (use_dem) {
            ...
            // Troviamo le coordinate mondiali del CENTRO ESATTO della mappa
            Point center_coords = asch.img2asc(Point(m_width / 2, m_depth / 2));
            double min_altitude = dem.min; 
            
            // Aggiungiamo uno zero: rimpiccioliamo 10 volte di più!
            float scale = 0.005f; 
            ...
            for (int z = 0; z < m_depth; ++z) {
                for (int x = 0; x < m_width; ++x) {
                    ...
                    // Sottraendo il centro, la mappa si costruisce a cavallo dello (0,0)
