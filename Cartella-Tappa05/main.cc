...
public:
    Camera (Shaders& shaders) {
        ...
        position = glm::vec3(25.0f, 15.0f, 50.0f); 
...
public:
    Scene () { 
        send_arrays_2a3f(); 
        generate(4, 0.05f, 10.0f, 12345); // Generazione di default
    }
    ~Scene () { clean (); }
...
int main() {
    ...
    while (running) {
        ...
        // Sezione 1: Generazione
        ImGui::TextColored(ImVec4(0.2f, 0.8f, 0.2f, 1.0f), "TERRENO PROCEDURALE");
        ...
        bool changed = false;
        if (ImGui::Button("Genera Nuovo (Cambia Seed)")) {
            current_seed = std::rand();
            changed = true;
        }
